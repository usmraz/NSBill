...
import { generateLedgerReport } from '../storage/pdfStorage';

export default function LedgerReportScreen({ navigation, route }) {
  ...
  const handleGenerate = async () => {
    ...
    if (!allDates && !fromDate && !toDate) {
      Alert.alert('Select Dates', 'Please select a date range or tap "All Dates".');
      return;
    }
      ...
      >
        ...
        <TouchableOpacity
          style={[styles.allDatesBtn,
            allDates && styles.allDatesBtnActive]}
          onPress={() => {
            setAllDates(true);
            setFromDate(null);
            setToDate(null);
        ...
        <View style={styles.previewCard}>
          ...
          <View style={styles.previewRow}>
            ...
            <Text style={styles.previewValue}>
              All bills + payments, chronological, farmer subtotals
            </Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  noCustomersText:            { padding: 20, textAlign: 'center', color: '#aaa' },
  ...
  previewValue: {
    fontSize: 12, color: '#222', fontWeight: '600',
